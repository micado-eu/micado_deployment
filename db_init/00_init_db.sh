...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" <<-EOSQL
    ...
    GRANT CREATE ON SCHEMA keycloak TO $POSTGRES_USER;
    ALTER DEFAULT PRIVILEGES IN SCHEMA keycloak GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO $POSTGRES_USER;
    ALTER DEFAULT PRIVILEGES IN SCHEMA keycloak GRANT USAGE ON SEQUENCES TO $POSTGRES_USER;

    CREATE SCHEMA micadoapp;
    GRANT USAGE ON SCHEMA micadoapp TO $POSTGRES_USER;
    GRANT CREATE ON SCHEMA micadoapp TO $POSTGRES_USER;
    ALTER DEFAULT PRIVILEGES IN SCHEMA micadoapp GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO $POSTGRES_USER;
    ALTER DEFAULT PRIVILEGES IN SCHEMA micadoapp GRANT USAGE ON SEQUENCES TO $POSTGRES_USER;

    CREATE SCHEMA gitea;
    CREATE USER $GITEA_DB_USER WITH ENCRYPTED PASSWORD '$GITEA_DB_PWD';
    GRANT CONNECT ON DATABASE $POSTGRES_DB TO $GITEA_DB_USER;
    GRANT USAGE ON SCHEMA gitea TO $GITEA_DB_USER;
    GRANT CREATE ON SCHEMA gitea TO $GITEA_DB_USER;
    ALTER ROLE $GITEA_DB_USER IN DATABASE $POSTGRES_DB SET search_path = gitea;
    ALTER DEFAULT PRIVILEGES IN SCHEMA gitea GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO $GITEA_DB_USER;
    ALTER DEFAULT PRIVILEGES IN SCHEMA gitea GRANT USAGE ON SEQUENCES TO $GITEA_DB_USER;

   CREATE EXTENSION IF NOT EXISTS pgroonga schema micadoapp;
   CREATE EXTENSION IF NOT EXISTS pgcrypto schema micadoapp;
   CREATE EXTENSION IF NOT EXISTS "uuid-ossp" schema micadoapp;
EOSQL
