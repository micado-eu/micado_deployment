...
(set -a; source prod.env; source .env; set +a; docker-compose -f docker-compose.yaml exec micado_db pg_dump -d $POSTGRES_DB -n $MICADO_DB_SCHEMA -U $POSTGRES_USER -Fc -c -v -b -x -O -f micado.sql)
echo "dump generated, saving file"
docker cp $(docker-compose -f docker-compose.yaml ps -q micado_db):/micado.sql micado.sql
